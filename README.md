...
Semantic HTML adds definition to the code and makes it easier for browsers to read.

2. Name two big differences between ```display: block;``` and ```display: inline;```.

Two big differences are that in display:block, a full line is taken up by a single element.  Display:inline does not take up the full line.
...
It will be on the cross-axis.

5. Explain why git is valuable to a team of developers.

Git is valuable by allowing a developer to work on a local computer, even if there is no internet.  It also allows the developer to keep track of any changes made and allows you to revert to a previously working repository.
